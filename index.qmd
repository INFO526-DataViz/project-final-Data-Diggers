...
Meteorites act as cosmic time capsules, preserving the mysteries of our solar system's formation. These celestial travelers have voyaged through space, bearing witness to ancient cosmic events, their rocky surfaces preserving stories of our cosmic past. Scattered across the universe, these wanderers hold untold narratives that illuminate the genesis of our solar system. Our latest venture introduces an interactive online platform, drawn from Javier de la Torre's dataset, dedicated to unveiling the captivating world of these celestial marvels. Our objective is to present an interactive visualization spotlighting the annual distribution of meteorite falls across diverse global regions. Through this visualization, users can explore and grasp the lasting impact of these cosmic visitors on Earth over time.

Our dataset encompasses 34,513 meteorites, offering essential insights including names, IDs, classifications, masses, fall occurrences, discovery years, and precise landing locations. Leveraging this data, our primary aim is to create an interactive display illustrating the yearly patterns of meteorite falls worldwide, painting a vivid picture of these cosmic wanderers' profound influence on Earth's history.
...
ggplot() +
  geom_sf(data = world, fill = "lightgrey") +
  geom_bin2d(data = meteorite, aes(x = reclong, y = reclat, fill = ..count..), bins = 170) +
