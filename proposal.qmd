...
The project aims to build an interactive website for visualizing asteroid impacts on Earth, using NASA's comprehensive asteroid data set. The focus is on creating a user-friendly platform that simplifies complex space data for educational and informative purposes.
...
The objective of this project is to develop an interactive web platform dedicated to visualizing data derived from asteroid impact discoveries. This will be achieved by utilizing information from two specific data sets: "Asteroid Names and Discovery V4.0" (DATA_SET_ID: EAR-A-5-DDR-ASTNAMES-DISCOVERY-V4.0) and the "Meteorite Landings" data set provided by NASA. The focus is on presenting this data in an engaging and accessible manner for users interested in astronomical phenomena and space research.
